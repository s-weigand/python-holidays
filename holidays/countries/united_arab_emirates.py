...
class UnitedArabEmirates(HolidayBase):
    ...
    def _populate(self, year):
        ...
        dates_obs = {2017: [(JUN, 25)], 2018: [(JUN, 14)], 2019: [(JUN, 3)], 2020: [(MAY, 24)]}
        fitr = "Eid al-Fitr"
        if year in dates_obs:
            ...
            for date_obs in get_gre_date(year, 10, 1):
                ...
                self[hol_date + rd(days=1)] = "{} Holiday* (*estimated)".format(fitr)
                self[hol_date + rd(days=2)] = "{} Holiday* (*estimated)".format(fitr)

        # Arafat Day & Eid al-Adha
        dates_obs = {2017: [(AUG, 31)], 2018: [(AUG, 20)], 2019: [(AUG, 10)], 2020: [(JUL, 30)]}
        ...
        if year in dates_obs:
            ...
            for date_obs in get_gre_date(year, 12, 9):
                ...
                self[hol_date + rd(days=2)] = "{}* Holiday* (*estimated)".format(adha)
                self[hol_date + rd(days=3)] = "{} Holiday* (*estimated)".format(adha)

        # Islamic New Year - (hijari_year, 1, 1)
        dates_obs = {2017: [(SEP, 22)], 2018: [(SEP, 11)], 2019: [(AUG, 31)], 2020: [(AUG, 23)]}
