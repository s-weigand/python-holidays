...
class Spain(HolidayBase):
    PROVINCES = [
        'AN', 'AR', 'AS', 'CB', 'CM', 'CL', 'CT', 'VC', 'EX', 'GA', 'IB', 'CN',
        'MD', 'MC', 'ML', 'NC', 'PV', 'RI'
    ]
    ...
    def _populate(self, year):
        ...
        self._is_observed(
            date(year, DEC, 6), "Día de la Constitución "
            "Española"
        )
        ...
        if self.prov:
            if self.prov == 'AN':
                ...
                self._is_observed(
                    date(year, JUL, 28), "Día de las Instituci"
                    "ones de Cantabria"
                )
            elif self.prov == 'CM':
                self._is_observed(
                    date(year, MAY, 31), "Día de Castilla "
                    "La Mancha"
                )
            elif self.prov == 'CL':
                self._is_observed(
                    date(year, APR, 23), "Día de Castilla y "
                    "Leon"
                )
            elif self.prov == 'CT':
                self._is_observed(
                    date(year, SEP, 11), "Día Nacional de "
                    "Catalunya"
                )
            elif self.prov == 'VC':
                self._is_observed(
                    date(year, OCT, 9), "Día de la Comunidad "
                    "Valenciana"
                )
            elif self.prov == 'EX':
                ...
                self._is_observed(
                    date(year, JUL, 25), "Día Nacional de "
                    "Galicia"
                )
            elif self.prov == 'IB':
                self._is_observed(
                    date(year, MAR, 1), "Día de las Islas "
                    "Baleares"
                )
            elif self.prov == 'CN':
                ...
                self._is_observed(
                    date(year, MAY, 2), "Día de Comunidad de "
                    "Madrid"
                )
            elif self.prov == 'MC':
                self._is_observed(
                    date(year, JUN, 9), "Día de la Región de "
                    "Murcia"
                )
            elif self.prov == 'NC':
